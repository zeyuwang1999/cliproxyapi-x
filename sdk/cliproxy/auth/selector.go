...
// SetConfig forwards runtime config updates to the wrapped selector when supported.
// This preserves nested selector behavior such as max-active-auths while session
// affinity is enabled.
func (s *SessionAffinitySelector) SetConfig(cfg *internalconfig.Config) {
	if s == nil || s.fallback == nil {
		return
	}
	if aware, ok := s.fallback.(interface{ SetConfig(*internalconfig.Config) }); ok {
		aware.SetConfig(cfg)
	}
}
